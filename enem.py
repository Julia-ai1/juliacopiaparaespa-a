...
def generate_questions(chat, pdf_content, num_questions):
    ...
    system_text = f"""Eres un asistente en portugués (brasil) que genera preguntas de opción múltiple. En caso de términos matemáticos, ponlos en formato entendible. Quiero que me generes preguntas con una estructura y contenido similar a las preguntas proporcionadas en el siguiente contexto {escaped_pdf_content}. Pon solo las preguntas y respuestas,NO HAGAS comentarios, NO PONGAS la respuesta correcta en las opciones. Coge la estructura, incluyendo en la pregunta inicial TODO el texto para formular la pregunta y las posibles opciones, como en el siguiente formato:
